...
% BATCH
BATCH_n_drops = [1 4 10 20];
BATCH_mem = [0.98 0.99 0.998];

% BATH
VAR_type = 'circular_well';
VAR_thresholdPerturbationType = 'bessel';
...
parfor i = 1:threadCount
    ...
    p.threshold_perturbation_type = VAR_thresholdPerturbationType; % options: 'bessel', 'deltas'
    ...
    if VAR_shouldOverrideThreshold
        ...
        thresholdCacheFolder = "";
        switch p.threshold_perturbation_type
            case 'gaussians'
                thresholdCacheFolder = "threshold_cache_gaussians";
            case 'bessel'
                thresholdCacheFolder = "threshold_cache";
        end
        thresholdFile = sprintf("%s/%s/%f_%f_%f.mat", BASE_DIRECTORY, thresholdCacheFolder, p.h0, p.h1, p.Rc);
