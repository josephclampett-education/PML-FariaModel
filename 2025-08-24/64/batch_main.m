...
% BATCH
BATCH_theta = [1.3];
BATCH_droplet_collision_type = ["none"; "spring"];
...
% VAR_droplet_collision_type = 'rigid';
...
count1 = length(BATCH_droplet_collision_type);
...
parfor i = 1:threadCount
    ...
    p.droplet_collision_type = BATCH_droplet_collision_type(idx1); % options: 'none', 'spring'
    ...
    outputSubfolder = sprintf("RES_N=%d, mem=%.2f, %s R=%.2f h0=%.2f h1=%.2f, theta=%.2f, %s", p.n_drops, p.mem * 100, p.topography_type, p.Rc, p.h0 * 1000, p.h1 * 1000, p.theta / pi, p.droplet_collision_type);
