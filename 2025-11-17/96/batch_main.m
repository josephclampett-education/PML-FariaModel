function batch_main(IN_batchIndex)
	...
	% DROPLETS
	VAR_r = (0.45)*10^(-3); % m
	VAR_theta = 1.35;       % (implicit *π)
	VAR_n_drops = 10;

	VAR_droplet_collision_type = "exp";
	VAR_droplet_collision_k = [1 2 6];
	VAR_droplet_collision_n = [2 6 12];
	...
	if isfile(BASE_DIRECTORY + "/ISLOCAL")
		...
		VAR_nimpacts = 40 * 60 * 2;
		VAR_n_save_wave = 1000;
	end

	% BATCH
	BATCH0_droplet_collision_k = VAR_droplet_collision_k;
	BATCH1_droplet_collision_n = VAR_droplet_collision_n;
	...
	count0 = length(VAR_droplet_collision_n);
	count1 = length(BATCH1_droplet_collision_n);
	...
	parfor i = 1:threadCount
		...
		switch (p.droplet_collision_type)
			...
			case "exp"
				p.droplet_collision_k = BATCH0_droplet_collision_k(idx0);
				p.droplet_collision_n = BATCH1_droplet_collision_n(idx1);
		end
		...
		outputSubfolder = sprintf("RES_N=%d, mem=%.2f, %s R=%.2f h0=%.2f h1=%.2f, theta=%.2f, c4=%.2f, k=%d, n=%d", p.n_drops, p.mem * 100, p.topography_type, p.Rc, p.h0 * 1000, p.h1 * 1000, p.theta / pi, p.c4, p.droplet_collision_k, p.droplet_collision_n);
