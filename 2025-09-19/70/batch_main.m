...
% BATCH
BATCH_effective_corral_radius_scale = [0.80 0.85 0.95 0.99];
BATCH_R = CONST_RLIST;
...
count0 = length(BATCH_effective_corral_radius_scale);
...
parfor i = 1:threadCount
    ...
    effective_corral_radius_scale = BATCH_effective_corral_radius_scale(idx0);
    switch p.damping_type
        case 'scaled'
            p.effective_corral_radius = radius * effective_corral_radius_scale;
    ...
    outputSubfolder = sprintf("RES_N=%d, mem=%.2f, %s R=%.2f h0=%.2f h1=%.2f, csr=%.2f csc=%.2f, theta=%.2f", p.n_drops, p.mem * 100, p.topography_type, p.Rc, p.h0 * 1000, p.h1 * 1000, p.effective_corral_radius / p.Rc, p.damping_scale, p.theta / pi);
