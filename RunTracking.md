...
2. Will increasing spatial/temporal resolution improve confinement?
	- 77. **TODO**: Default, N = 10, 10x temporal resolution. NOTE: not a batch run.
	- 78. **TODO** Default, maybe higher spatial resolution? NOTE: not a batch run.
3. Is the domain size large enough to damp waves outside bath for flat topography + rigid damping configuration? For one? Many?
	- 79. **TODO**: Default, N = [1, 10], 2x domain size
4. Compare free walking speed from experiments & simulations using many- and single-droplet speed histograms
