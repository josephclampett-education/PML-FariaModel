...
BATCH_initialXRatio = [0.0 0.5 0.95 1.0];
...
count1 = length(BATCH_initialXRatio);
...
parfor i = 1:threadCount
    ...
    p.xi = p.Rc * BATCH_initialXRatio(idx1);
    p.yi = 0;
    p.ui = 0;
    p.vi = 0;
    ...
    outputSubfolder = sprintf("RES_N=%d, mem=%.2f, %s R=%.2f h0=%.2f h1=%.2f, theta=%.2f, x=%.2f", p.n_drops, p.mem * 100, p.topography_type, p.Rc, p.h0 * 1000, p.h1 * 1000, p.theta / pi, p.Rc * BATCH_initialXRatio(idx1));
